...
```bash
# 后端 API 地址（运行时注入，必须为完整 URL，包含 https://）
...
> 将 `Backend` 替换为你的后端服务名（如 `incredible-balance-production-791d` 等）
> 示例：`API_BASE_URL=https://incredible-balance-production-791d.up.railway.app/api`
>
> ⚠️ **apiBaseUrl 必须为完整 URL**：如 `https://backend.railway.app/api`。若只写 `xxx.up.railway.app/api`（无协议），请求会被当作相对路径，导致返回 HTML 而非 API 数据。
...
| 变量 | 说明 |
|------|------|
| `API_BASE_URL` | **必填**。后端 API 完整地址，必须包含 `https://`，如 `https://xxx.up.railway.app/api`。若缺少协议，请求会失败。 |
| `VITE_API_BASE_URL` | 可选，构建时备用；运行时以 `API_BASE_URL` 为准 |
